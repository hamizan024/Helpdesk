...
<x-app-card title="Departments" icon="business" :badge="$items->total()" :noPad="true">
    <x-slot:actions>
        <div class="d-flex align-items-center gap-2">
            {{-- Search --}}
            <form method="GET" action="{{ route('master.departments.index') }}" class="d-flex">
                <div class="input-group input-group-sm" style="width:220px;">
                    ...
                    @if($search)
                        <a href="{{ route('master.departments.index') }}" class="btn btn-outline-secondary" title="Clear">
            ...
            <tbody>
                @forelse($items as $dept)
                    <tr>
                        ...
                        <td class="px-4 py-3 fw-semibold" style="color:#344767;">{{ $dept->name }}</td>
                        <td class="px-4 py-3" style="color:#7b809a;max-width:300px;">
                            {{ $dept->description ?? '—' }}
                        </td>
                        <td class="px-4 py-3">
                            @if($dept->is_active)
                        ...
                        <td class="px-4 py-3 text-end">
                            <button type="button"
                                    onclick="openEdit({{ $dept->id }}, @json($dept->name), @json($dept->description ?? ''), {{ $dept->is_active ? 'true' : 'false' }})"
                                    class="btn btn-sm btn-outline-primary me-1"
                                    style="font-size:0.75rem;padding:3px 10px;">
                                Edit
                            </button>
                            <button type="button"
                                    onclick="openDelete({{ $dept->id }}, @json($dept->name))"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-4 text-center text-muted">
                            {{ $search ? 'No departments match your search.' : 'No departments yet. Click Add to create the first one.' }}
                        </td>
                    </tr>
                @endforelse
            </tbody>
        ...
        <div class="modal-content">
            <form id="dataForm" method="POST" action="">
                ...
                <div class="modal-header" style="background:linear-gradient(195deg,#42424a,#191919);border-radius:8px 8px 0 0;">
                    <h6 class="modal-title text-white fw-bold" id="formModalTitle">Add Department</h6>
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" id="fieldName" name="name"
                               ...
                               placeholder="Department name" maxlength="100" required>
...
@section('scripts')
<script>
const storeUrl  = '{{ route("master.departments.store") }}';
const updateUrl = (id) => `/master/departments/${id}`;
const deleteUrl = (id) => `/master/departments/${id}`;

function openCreate() {
    document.getElementById('formModalTitle').textContent = 'Add Department';
...
function openEdit(id, name, description, isActive) {
    document.getElementById('formModalTitle').textContent = 'Edit Department';
